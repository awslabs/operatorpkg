...
func (u UnstructuredAdapter) GetConditions() []Condition {
	...
	return lo.Map(conditions, func(condition interface{}, _ int) Condition {
		...
		lo.Must0(json.Unmarshal(jsonStr, &newCondition))
		return newCondition
	})
}
func (u UnstructuredAdapter) SetConditions(conditions []Condition) {
	lo.Must0(unstructured.SetNestedSlice(u.Object, lo.Map(conditions, func(condition Condition, _ int) interface{} {
		var b map[string]interface{}
		j, _ := json.Marshal(&condition)
		lo.Must0(json.Unmarshal(j, &b))
		return b
	}), "status", "conditions"))
}
